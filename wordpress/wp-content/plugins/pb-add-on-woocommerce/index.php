 ...
 * Text Domain: Optional. Plugin's text domain for localization:  profile-builder
...
if ( ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) || (is_plugin_active_for_network('woocommerce/woocommerce.php')) )  {
    ...
    function wppb_woo_clear_session_on_save( $request, $form_name, $user_id ){
    ...
    add_action( 'wppb_edit_profile_success', 'wppb_woo_clear_session_on_save', 20, 4 );
	...
	function wppb_woo_add_scripts_front_end() {
	...
	add_action('wp_enqueue_scripts' , 'wppb_woo_add_scripts_front_end');
     ...
     */
    add_filter('wppb_userlisting_get_merge_tags', 'wppb_woo_add_userlisting_tags', 10, 2);  //add tags to User Listing
    function wppb_woo_add_userlisting_tags( $merge_tags, $type = '' ){
        ...
        $billing_fields_array = wppb_woo_get_billing_fields();
        foreach ( $billing_fields_array as $key => $value ) {
        ...
        $shipping_fields_array = wppb_woo_get_shipping_fields();
        foreach ( $shipping_fields_array as $key => $value ) {
     ...
     */
    add_filter('mustache_variable_sort_tag', 'wppb_woo_userlisting_sort_tags', 11, 4);
    function wppb_woo_userlisting_sort_tags( $value, $name, $children, $extra_info ){
     ...
     */
    function wppb_woo_get_billing_shipping_country_state_full_name($value, $name, $children, $extra_values){
    ...
    add_filter('mustache_variable_user_meta', 'wppb_woo_get_billing_shipping_country_state_full_name', 12, 4); //give it a lower priority to be executed last


    /*
    * Display (Billing & Shipping) Country and State full name (not just codes) in Facet filters
    */
    function wppb_woo_billing_shipping_country_state_name( $returned_value, $meta_value, $faceted_filter_options, $wppb_manage_fields ){
        ...
        if ( ($faceted_filter_options['facet-meta'] == 'billing_state') || ($faceted_filter_options['facet-meta'] == 'shipping_state') ) {
            ...
            if ( class_exists('WC_Countries') ) {
                ...
                if (isset($_GET['ul_filter_billing_country']))
                    $country_code = $_GET['ul_filter_billing_country'];
                else if (isset($_GET['ul_filter_shipping_country']))
                    $country_code = $_GET['ul_filter_shipping_country'];
    ...
    add_filter('wppb_ul_facet_value_or_label', 'wppb_woo_billing_shipping_country_state_name', 10, 4);
    ...
    */
    function wppb_woo_empty_results_if_no_country_selected($results, $facet_meta, $faceted_filters, $wppb_manage_fields){
    ...
    add_filter('wppb_get_all_values_for_user_meta' , 'wppb_woo_empty_results_if_no_country_selected', 10, 4);
     ...
     */
    function wppb_woo_custom_no_options_message_billing_shipping_state( $message, $faceted_filter_options ){
    ...
    add_filter('wppb_facet_no_options_message', 'wppb_woo_custom_no_options_message_billing_shipping_state', 10, 2);


    /*
     * Include individual Woo Shipping Address & Billing Address fields in Search Fields and Faceted Menus (as facet meta)
     */
    function wppb_woo_include_billing_shipping_in_search_all_fields_and_faceted_menus( $fields ){
    ...
    add_filter( 'wppb_userlisting_search_all_fields', 'wppb_woo_include_billing_shipping_in_search_all_fields_and_faceted_menus' );
    add_filter( 'wppb_userlisting_facet_meta', 'wppb_woo_include_billing_shipping_in_search_all_fields_and_faceted_menus' );


    /*
     * Populate "Default Sorting Criteria" dropdown with individual Woo Shipping & Billing Address fields
     */
    function wppb_woo_include_billing_shipping_fields_in_default_sorting_criteria ($sorting_criteria) {
        $billing_fields_array = wppb_woo_get_billing_fields();
    ...
    add_filter('wppb_default_sorting_criteria', 'wppb_woo_include_billing_shipping_fields_in_default_sorting_criteria');


    /*
     * We also need to include individual Woo Billing & Shipping meta in user_meta_keys array
     */
    function wppb_woo_include_billing_shipping_in_search( $user_meta_keys ){
    ...
    add_filter( 'wppb_userlisting_search_in_user_meta_keys', 'wppb_woo_include_billing_shipping_in_search' );
    ...
    function wppb_woo_admin_notice() {
        ...
        <div class="update-nag">
            <?php _e( 'WooCommerce needs to be installed and activated for Profile Builder - WooCommerce Sync Add-on to work!', 'profile-builder' ); ?>
        </div>
    <?php
    }
