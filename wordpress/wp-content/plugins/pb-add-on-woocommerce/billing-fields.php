...
function wppb_woo_get_billing_fields(){
    return array(
...
function wppb_woo_billing_fields_array() {

    $billing_fields_array = wppb_woo_get_billing_fields();

    $field = wppb_woo_get_field('WooCommerce Customer Billing Address');
...
function wppb_woo_billing_state_not_required( $fields ){
...
add_filter('wppb_woo_billing_fields','wppb_woo_billing_state_not_required');

/* handle field output */
function wppb_woo_billing_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
    if ( $field['field'] == 'WooCommerce Customer Billing Address' ){
        ...
        if ( class_exists('WC_Countries') ) {
            ...
            foreach ($billing_fields as $field_key => $field_val) {
                ...
                $is_error = wppb_check_woo_individual_fields_val('', $field_val, $field_key, $request_data, $form_location);
                $error_class = '';
                ...
                if ($field_key == 'billing_country') {
                    ...
                    $output .= '<option value="" selected >' . esc_attr__('Select an option&hellip;', 'woocommerce') . '</option>';
        ...
        if ( ($form_location == 'register') && $in_manage_fields ) {
            ?><script type="text/javascript">
                jQuery(document).ready(function(){
                    wppb_woo_toggle_required_attrbute();
                    jQuery("#woo_different_shipping_address").click(function(){
                        wppb_woo_toggle_required_attrbute();
                        jQuery(".wppb-woocommerce-customer-shipping-address").toggle();
                    });
                    function wppb_woo_toggle_required_attrbute(){
...
add_filter( 'wppb_output_form_field_woocommerce-customer-billing-address', 'wppb_woo_billing_handler', 10, 6 );


/* handle field save */
function wppb_save_woo_billing_value( $field, $user_id, $request_data, $form_location ){
    if( $field['field'] == 'WooCommerce Customer Billing Address' ) {
        $billing_fields = wppb_woo_billing_fields_array();
        foreach ($billing_fields as $field_key => $field_val) {
...
add_action( 'wppb_save_form_field', 'wppb_save_woo_billing_value', 10, 4 );


/* handle field validation, to not save data in case some required fields are not filled in */
function wppb_check_woo_billing_value( $message, $field, $request_data, $form_location ){
...
add_filter( 'wppb_check_form_field_woocommerce-customer-billing-address', 'wppb_check_woo_billing_value', 10, 4 );


/* Add billing information to wp_signups table (when Email Confirmation is active) */
function wppb_woo_meta_activation_billing( $posted_value, $field, $global_request ){
    $billing_fields = wppb_woo_billing_fields_array();
    $return_values = array();
    foreach ($billing_fields as $field_key => $field_val) {
...
add_filter( 'wppb_add_to_user_signup_form_field_woocommerce-customer-billing-address', 'wppb_woo_meta_activation_billing',10, 3 );


/* Add Woo Billing meta on user activation*/
function wppb_woo_adding_billing_meta_on_activation($user_id, $password, $meta){
    $billing_fields = wppb_woo_billing_fields_array();
...
add_action( 'wppb_add_meta_on_user_activation_woocommerce-customer-billing-address', 'wppb_woo_adding_billing_meta_on_activation', 10, 3 );
