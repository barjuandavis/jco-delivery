...
function wppb_woo_get_shipping_fields(){
    return array(
...
function wppb_woo_shipping_fields_array(){

    $shipping_fields_array = wppb_woo_get_shipping_fields();

    $field = wppb_woo_get_field('WooCommerce Customer Shipping Address');
...
function wppb_woo_shipping_state_not_required( $fields ){
...
add_filter('wppb_woo_shipping_fields','wppb_woo_shipping_state_not_required');

/* handle field output */
function wppb_woo_shipping_handler( $output, $form_location, $field, $user_id, $field_check_errors, $request_data ){
...
add_filter( 'wppb_output_form_field_woocommerce-customer-shipping-address', 'wppb_woo_shipping_handler', 10, 6 );


/* handle field save */
function wppb_save_woo_shipping_value( $field, $user_id, $request_data, $form_location ){
    if( $field['field'] == 'WooCommerce Customer Shipping Address' ){
        $shipping_fields = wppb_woo_shipping_fields_array();
...
add_action( 'wppb_save_form_field', 'wppb_save_woo_shipping_value', 10, 4 );


/* handle field validation, to not save data in case some required fields are not filled in */
function wppb_check_woo_shipping_value( $message, $field, $request_data, $form_location ){
...
add_filter( 'wppb_check_form_field_woocommerce-customer-shipping-address', 'wppb_check_woo_shipping_value', 10, 4 );


/* Add shipping information to wp_signups table (when Email Confirmation is active)*/
function wppb_woo_meta_activation_shipping( $posted_value, $field, $global_request ){
    $shipping_fields = wppb_woo_shipping_fields_array();
    $return_values = array();
    foreach ($shipping_fields as $field_key => $field_val) {
...
add_filter( 'wppb_add_to_user_signup_form_field_woocommerce-customer-shipping-address', 'wppb_woo_meta_activation_shipping',10, 3 );


/* Add Woo Shipping meta on user activation*/
function wppb_woo_adding_shipping_meta_on_activation($user_id, $password, $meta){
    $shipping_fields = wppb_woo_shipping_fields_array();
...
add_action( 'wppb_add_meta_on_user_activation_woocommerce-customer-shipping-address', 'wppb_woo_adding_shipping_meta_on_activation', 10, 3 );
