...
define( 'eb_google_map_version', '1.1.5' );
...
function eb_google_map_extended() {
	...
	load_plugin_textdomain( 'extended-google-map-for-elementor', FALSE, basename( dirname( __FILE__ ) ) . '/languages' );
...
function eb_elementor_google_map_fail_load() {
	...
	$message = '<p>' . __( 'You do not have Elementor Page Builder on your WordPress. Elementor Google Map Extended require Elementor in order to work.', 'extended-google-map-for-elementor' ) . '</p>';
...
function eb_check_google_map_elementor_load_out_of_date() {
	...
	$message = '<p>' . __( 'Elementor Google Map Extended may not work or is not compatible because you are using an old version of Elementor.', 'extended-google-map-for-elementor' ) . '</p>';
	$message .= '<p>' . sprintf( '<a href="%s" class="button-primary">%s</a>', $upgrade_link, __( 'Update Elementor Now', 'extended-google-map-for-elementor' ) ) . '</p>';
...
add_action('elementor/editor/before_enqueue_scripts', function(){
	...
	wp_enqueue_script( 'eb-google-maps-api-admin', 'https://maps.googleapis.com/maps/api/js?key=' . eb_map_get_option( 'eb_google_map_api_key', 'eb_map_general_settings' ) . '&language=' . eb_map_get_option( 'eb_google_map_lang', 'eb_map_general_settings' ) . '', ['jquery'], eb_google_map_version, true  );
...
add_action('elementor/frontend/after_register_scripts', function(){
	wp_register_script( 'eb-google-maps-api', 'https://maps.googleapis.com/maps/api/js?key=' . eb_map_get_option( 'eb_google_map_api_key', 'eb_map_general_settings' ) . '&language=' . eb_map_get_option( 'eb_google_map_lang', 'eb_map_general_settings' ) . '', [], eb_google_map_version, true  );
